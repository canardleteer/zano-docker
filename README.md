...
```shell
# To build Zano 2.2.1.506 (the default ZANO_REF) with 1 core
docker build -t zano:latest .

# To build Zano 2.2.1.506 with all processors on your system.
docker build \
    --build-arg ZANO_REF="2.2.1.506" \
    --build-arg BUILD_WIDTH=$(nproc) \
    -t zano-runner:latest .

# To build a distroless zanod from Zano 2.2.1.506 with all processors on
# your system.
docker build \
    --build-arg ZANO_REF="2.2.1.506" \
    --build-arg BUILD_WIDTH=$(nproc) \
...
# To build a distroless simplewallet from Zano 2.2.1.506 with all processors
# on your system.
docker build \
    --build-arg ZANO_REF="2.2.1.506" \
    --build-arg BUILD_WIDTH=$(nproc) \
